...
int evil_mode;                // Nonzero iff this peer should behave badly.
                            // If evil_mode == 1, this peer will randomly choose
                            // a different evil mode on each upload request.
                            // If evil_mode > 1 && evil_mode < EVIL_NUM, every
                            // upload request will use the same attack.
                            // We have provided code to select the propper evil
                            // attack for uploads.  For download attacks, extend
                            // the eviltype_t enum and follow the above rules.
                            // You are welcome to associate each evil mode with
                            // whatever form of evil you desire.

// The set of enums for performing evil attacks.  
typedef enum {
    // Perform no attacks.
    GOOD = 0,
    // The set of upload attacks.
    EVIL_ONE = 1, EVIL_TWO, EVIL_THREE,
    // The total number of evil modes.
    EVIL_NUM,
} eviltype_t;
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 3 && strcmp(argv[1], "-t") == 0
        ...
        evil_mode = strtol(argv[2], NULL, 10);
        argc -= 2, argv += 2;
        goto argprocess;
